/**
 * Diagnostic Express questions — Phase K1A (worker copy).
 *
 * Mirror of src/data/diagnostic-questions.ts. Kept in sync manually until
 * a shared-data layer is added in a later phase.
 *
 * Used server-side for:
 *   - validating that submitted answer values match the canonical option set
 *   - computing the score (raw weighted sum)
 *   - computing the bucket from the normalized score
 *
 * NEVER trust client-submitted scores — score is always recomputed here.
 */
