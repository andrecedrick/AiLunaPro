...
import { validateAdvancedInputs, scoreRoiAdvanced, type RoiAdvancedInputs } from '../lib/roi-advanced';
...
/**
 * Advanced ROI (G5) — full time→money breakdown (loaded rate, team-scale,
 * Cost-of-Inaction, frais → payback/ROI%, freed FTE, optional revenue).
 *
 *   POST /api/public/roi-advanced-calculation
 *
 * Same anti-abuse + lead + persistence model as the K2A endpoint above.
 * Server-authoritative: client-submitted result fields are ignored.
 */
roi.post('/api/public/roi-advanced-calculation', async c => {
  const env = c.env as AppEnv['Bindings'] & {
    FIREBASE_SERVICE_ACCOUNT_JSON?: string;
    TURNSTILE_SECRET_KEY?:          string;
  };
  ...
  const inputsErr = validateAdvancedInputs(body.inputs);
  if (inputsErr) {
    return c.json({ error: inputsErr, code: 'INVALID_INPUTS' }, 400);
  }
  const inputs = body.inputs as RoiAdvancedInputs;
  ...
  const scored = scoreRoiAdvanced(inputs);
  const result = scored.result;
  ...
  const doc = {
    ...
    cf: { country: country ?? null, ipHash: ipHash ?? null },
    schemaVersion: 1,
    createdAt:     now,
    expiresAt:     fsTimestamp(expiresAtFromNow()),
  };

  try {
    await firestoreSet(env.FIREBASE_SERVICE_ACCOUNT_JSON, `public_roi_calculations/${id}`, doc as unknown as Parameters<typeof firestoreSet>[2]);
  } catch (err) {
    console.error('[roi-advanced] firestoreSet failed:', err);
    return c.json({ error: 'Failed to save ROI estimate', code: 'PERSIST_FAILED' }, 500);
  }

  dlog(env,
    '[roi-advanced] saved — id:', id,
    'workflow:', inputs.targetWorkflow,
    'teamYearlySaved:', result.teamYearlyCostSaved,
    'coiPerYear:', result.coiPerYear,
