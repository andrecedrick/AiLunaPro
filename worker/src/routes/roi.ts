...
/** Hard per-IP daily ceiling on persisted public ROI writes (abuse / cost cap). */
const ROI_DAILY_CAP = 50;
...
roi.post('/api/public/roi-calculation', async c => {
  ...
  // Do NOT trust X-Forwarded-For (attacker-controlled — rotating it would defeat
  ...
  // Absolute per-IP daily ceiling — bounds total persisted writes (cooldown bounds
  // rate, not total). Each accepted call does a 90-day-TTL firestoreSet.
  if (remoteIp) {
    const cap = await checkDailyCap(env.FIREBASE_SERVICE_ACCOUNT_JSON, 'roi_day', remoteIp, ROI_DAILY_CAP);
    if (!cap.ok) {
      return c.json({ error: 'Daily limit reached. Please try again tomorrow.', code: 'DAILY_CAP' }, 429);
    }
  }
