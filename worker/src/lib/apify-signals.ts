/**
 * Apify dataset items → raw observations (cahier §26.4, §26.6 rule 2).
 *
 * PURE and DETERMINISTIC. Same page text in, same observations out, in the same
 * order. This is the extraction half of the collection stage: it decides what was
 * OBSERVED, never what it means. No scoring, no findings, no recommendations —
 * those belong to Phases 4 and 5.
 *
 * Detection is pattern-based rather than model-based on purpose. Extraction may
 * legitimately use a model (§26.6 rule 2), but a rule table is reproducible for
 * free, and reproducibility is the property the whole engine is built on. A model
 * pass can be added later as an additional collector without changing this one.
 *
...
import type { RawObservation, SourceType, SignalType } from './enrichment-evidence';

/** Window of surrounding text kept as the captured excerpt. */
const EXCERPT_WINDOW = 240;
/** Cap per page, so one enormous page cannot dominate a snapshot. */
const MAX_OBSERVATIONS_PER_ITEM = 25;

/**
 * Known AI vendors.
 *
 * `patterns` match a script src, a domain or a product name in page text.
...
interface Match { signalType: SignalType; observedValue: string; index: number; length: number }
...
export function detectSignals(text: string, sourceType: SourceType): Match[] {
  ...
  for (const vendor of AI_VENDORS) {
    for (const re of vendor.patterns) {
      ...
      break;   // one observation per vendor per page
    }
  }
  ...
  if (sourceType === 'repository' || sourceType === 'github') {
 ...
 *
 * Items with no URL or no text yield nothing — an observation with no locatable
 * source cannot become evidence (enrichment-evidence rejects it anyway, but
 * discarding here keeps the snapshot free of unusable rows).
 */
export function mapItem(item: ApifyItem, opts: MapOptions): RawObservation[] {
  ...
  return detectSignals(text, opts.sourceType).map(m => ({
    sourceUrl:        url,
    sourceType:       opts.sourceType,
    collector:        'apify' as const,
    collectorRunId:   opts.collectorRunId,
    capturedEvidence: excerptAround(text, m.index, m.length),
