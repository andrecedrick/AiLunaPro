...
export class PdfBuilder {
  ...
  h1(text: string): void { this.spacer(2); this.drawWrapped(text, 'bold', 24, VIOLET, 7); }
  h2(text: string): void {
    this.ensure(38); this.spacer(16);
    this.drawWrapped(text, 'bold', 16.5, INK, 6);
    this.y -= 4; this.rectAbs(MARGIN, this.y, 42, 2.5, VIOLET); this.y -= 11; // brand accent rule (hierarchy)
  }
  ...
  muted(text: string, size = 9.5): void { this.drawWrapped(text, 'regular', size, MUTED, 4); this.y -= 1; }
  ...
  callout(text: string, bg: 'amber' | 'tint' = 'amber'): void {
    const size = 10.5, lineGap = 4, pad = 13;
