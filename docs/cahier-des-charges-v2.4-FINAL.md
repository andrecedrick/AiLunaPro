...
## 0bis. STATUS DASHBOARD *(source de vérité — docs-only, MAJ 31 mai 2026)*

> Factuel + aligné gating/NFR v2.4. Prod live : fe `index-CdRkHlbM.js` (deploy `05208d72`).
> Légende statut : **Shipped** (livré+prod) · **Partial** (partiel/lite) · **Planned** (spec, non implémenté).

### 0bis.1 Parcours 9 étapes (orchestré Luna) + mapping modules
```
...
### 0bis.2 Modules / features — statut
...
| **Audit Express (§3bis)** | public <5min demo, no account | **Partial (Phase 4: tap-path + Turnstile)** | surface + tap-flow; compute-only `POST /preview` (no PII/persistence) Turnstile-gated; `GET /config` serves public site key; K1A-lite + K2A-lite via Phase 0 engines, traced; 16 tests | V1-lite URL crawl, X1-lite OPEX, PDF export, account save — later. **Operator:** set Worker `TURNSTILE_SITE_KEY` |
...
### 0bis.3 Roadmap J1.x → statut + dépendances
...
> Note : ordre §7 **inchangé** ; pull-forward demo/SEO/PDF = **proposition v2.4 non encore gatée** (attend approbation scope).
