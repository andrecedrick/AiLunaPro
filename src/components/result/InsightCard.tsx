...
export function InsightCard(p: InsightCardProps) {
  ...
  return (
    <div style={card}>
      ...
      <Label>Example — how a similar business benefits</Label>
      <p style={{ ...body, fontStyle: 'italic' }}>{p.example} <span style={{ color: 'var(--text-muted)', fontStyle: 'normal' }}>(Illustrative.)</span></p>
      ...
      <div style={{ marginTop: 14, padding: '12px 14px', borderRadius: 12, background: 'var(--brand-soft-bg, var(--surface-2))', border: '1px solid var(--border)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginBottom: 6 }}>
          <span style={{ fontSize: 11, fontWeight: 800, letterSpacing: 1, textTransform: 'uppercase', color: accent }}>Do this next{p.doNext.heading ? ` — ${p.doNext.heading}` : ''}</span>
        ...
        {p.doNext.outcome && <p style={{ margin: '8px 0 0', fontSize: 12.5, color: 'var(--text-secondary)' }}>✓ Once done: {p.doNext.outcome}</p>}
      ...
      {p.references && p.references.length > 0 && (
        <p style={{ margin: '10px 0 0', fontSize: 11, color: 'var(--text-muted)' }}>
          References: {p.references.join(' · ')} <span style={{ opacity: 0.8 }}>(advisory, not legal advice)</span>
        </p>
      )}
    </div>
  );
